			...
			<table class="table table-striped"
				...
				<tbody>
					<tr>
						...
						<td colspan="2"><%= comm.getCommTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">","&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
					<tr>
						<td>작성자</td>
					...
					<tr>
						<td>작성일자</td>
						<td colspan="2"><%= comm.getCommDate().substring(0, 11) + comm.getCommDate().substring(11, 13) + "시 "
									+ comm.getCommDate().substring(14, 16) + "분"%></td>
					</tr>
					<tr>
						<td>내용</td>
						<td colspan="2" style="min-height: 200px; text-align: left;"><%= comm.getCommContent().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">","&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
				</tbody>
			</table>
			<a href="comm.jsp" class="btn btn-primary">목록</a>
			<%
					if(userID != null && userID.equals(comm.getUserID())){
			%>
					<a href="commUpdate.jsp?commID=<%=commID%>" class="btn btn-primary">수정</a>
					<a onclick="return confirm('삭제하시겠습니까?')" href="commDeleteAction.jsp?commID=<%=commID%>" class="btn btn-primary">삭제</a>
			<%			
					}
			%>
		</div>
